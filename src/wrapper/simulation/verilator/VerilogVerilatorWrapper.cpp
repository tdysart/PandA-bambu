 ...
 */
/**
 * @file VerilogVerilatorWrapper.cpp
...
VerilogVerilatorWrapper::VerilogVerilatorWrapper(const ParameterConstRef& _Param, const std::string& _top_fname,
                                                 const std::string& _inc_dirs)
    : SimulationTool(_Param, _top_fname, _inc_dirs)
{
   PRINT_OUT_MEX(OUTPUT_LEVEL_PEDANTIC, output_level, "Creating the Verilog testbench VERILATOR wrapper...");
}

std::string VerilogVerilatorWrapper::GenerateScript(std::ostream& script, const std::string& top_filename,
...
std::string VerilogVerilatorWrapper::GenerateSimulationScript(const std::string& top_filename,
                                                              std::list<std::string> file_list)
{
   ...
   script << "#!/bin/bash\n"
          ...
          << "# Self-contained Verilog testbench (--testbench-style=verilog)\n"
          << "set -e\n"
...
void VerilogVerilatorWrapper::Simulate(unsigned long long& accum_cycles, unsigned long long& n_testcases)
{
   ...
   DetermineVerilogCycles(accum_cycles, n_testcases);
}

void VerilogVerilatorWrapper::DetermineVerilogCycles(unsigned long long& accum_cycles, unsigned long long& n_testcases)
{
