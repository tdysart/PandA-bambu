...
void BambuParameter::PrintHelp(std::ostream& os) const
{
   ...
   os << "    --pretty-print=<file>.c\n"
      ...
      << "            legacy - Self-contained Verilog testbench driven by an XML test\n"
      << "                     vector, as generated by bambu 2023.1 and earlier. It has\n"
      << "                     no DPI-C dependency. Requires an XML --generate-tb file\n"
      << "                     with expected outputs, the minimal interface, and\n"
      << "                     --simulator=VERILATOR.\n"
      ...
      switch(opt)
      {
         ...
         case 0:
         {
            ...
            if(strcmp(long_options[option_index].name, "testbench-style") == 0)
            {
               ...
               if(style != "dpi" && style != "legacy" && style != "both")
               {
                  THROW_ERROR("BadParameters: testbench style not valid (expected dpi, legacy or both)");
               }
...
void BambuParameter::CheckParameters()
{
   ...
   /// The legacy (bambu 2023.1) testbench generator is only ported for this configuration so far
