...
void BambuParameter::PrintHelp(std::ostream& os) const
{
   ...
   os << "    --pretty-print=<file>.c\n"
      ...
      << "    --testbench-style=<style>\n"
      << "        Testbench generator used with --generate-tb:\n"
      << "            dpi     - DPI-C co-simulation testbench (default)\n"
      << "            verilog - Self-contained Verilog testbench driven by an XML test\n"
      << "                      vector, as generated by bambu 2023.1 and earlier. It has\n"
      << "                      no DPI-C dependency. Requires an XML --generate-tb file,\n"
      << "                      the minimal interface, and --simulator=VERILATOR.\n"
      << "                      Expected outputs missing from the XML are computed by\n"
      << "                      executing the specification on the host.\n"
      << "            both    - Generate both; simulation uses the DPI-C testbench.\n\n"
      << "    --top-fname=<fun_name>\n"
...
int BambuParameter::Exec()
{
   ...
   /// Scheduling options
       ...
       {"testbench-style", required_argument, nullptr, 0},
   ...
   while((opt = getopt_long(argc, argv, short_options, long_options, &option_index)) != -1)
   {
      switch(opt)
      {
         ...
         case 0:
         {
            ...
            if(strcmp(long_options[option_index].name, "testbench-style") == 0)
            {
               const std::string style(optarg);
               if(style != "dpi" && style != "verilog" && style != "both")
               {
                  THROW_ERROR("BadParameters: testbench style not valid (expected dpi, verilog or both)");
               }
               setOption(OPT_testbench_style, style);
               break;
            }
#if(__GNUC__ >= 7)
            [[gnu::fallthrough]];
...
void BambuParameter::CheckParameters()
{
   ...
   /// The self-contained Verilog testbench generator (from bambu 2023.1) is only ported for this configuration so far
   if(getOption<bool>(OPT_generate_testbench) && getOption<std::string>(OPT_testbench_style) != "dpi")
   {
      if(getOption<std::string>(OPT_simulator) != "VERILATOR")
      {
         THROW_ERROR("BadParameters: --testbench-style=" + getOption<std::string>(OPT_testbench_style) +
                     " requires --simulator=VERILATOR");
      }
      if(getOption<HLSFlowStep_Type>(OPT_interface_type) != HLSFlowStep_Type::MINIMAL_INTERFACE_GENERATION)
      {
         THROW_ERROR("BadParameters: --testbench-style=" + getOption<std::string>(OPT_testbench_style) +
                     " requires the minimal interface");
      }
      const auto tb_file =
          isOption(OPT_testbench_input_file) ? getOption<std::string>(OPT_testbench_input_file) : std::string();
      if(!ends_with(tb_file, ".xml") || tb_file.find(STR_CST_string_separator) != std::string::npos)
      {
         THROW_ERROR("BadParameters: --testbench-style=" + getOption<std::string>(OPT_testbench_style) +
                     " requires a single XML --generate-tb file");
      }
   }
...
void BambuParameter::SetDefaults()
{
   ...
   setOption(OPT_testbench_style, "dpi");
