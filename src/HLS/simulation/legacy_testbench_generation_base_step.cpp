...
LegacyTestbenchGenerationBaseStep::LegacyTestbenchGenerationBaseStep(
    ...
    : HLS_step(_parameters, _HLSMgr, _design_flow_manager, _hls_flow_step_type),
      ...
      mod(nullptr),
      target_period(0.0),
      output_directory((parameters->getOption<std::filesystem::path>(OPT_output_directory) / "simulation").string() +
                       "/"),
...
void LegacyTestbenchGenerationBaseStep::Initialize()
{
   ...
   const auto top_function_id = HLSMgr->get_tree_manager()->GetFunction(top_symbols.front())->index;
...
std::string LegacyTestbenchGenerationBaseStep::verilator_testbench() const
{
   ...
   if(!std::filesystem::exists(simulation_values_path))
   {
      ...
      interface_type == HLSFlowStep_Type::INTERFACE_CS_GENERATION)
   {
      ...
      for(const auto& par : DesignSignature)
      {
         ...
         if(InterfaceType == port_o::port_interface::PI_DEFAULT)
         {
            ...
            bool is_real;
            const auto pi_node = TM->GetTreeNode(portInst->get_typeRef()->treenode);
...
void LegacyTestbenchGenerationBaseStep::write_module_instantiation(bool xilinx_isim) const
{
   ...
   const auto target_writer = target_language == HDLWriter_Language::VERILOG ?
                                  ...
                                  language_writer::create_writer(
                                      target_language, HLSMgr->get_HLS_device()->get_technology_manager(), parameters);
...
void LegacyTestbenchGenerationBaseStep::write_auxiliary_signal_declaration() const
{
   ...
   /// In 2023.1 interface type attributes existed only for inferred (or pragma-specified) interfaces; 2024
   /// always populates module_arch, so key this on the interface generation mode instead
   ...
   if(mod->get_out_port_size())
   {
      for(unsigned int i = 0; i < mod->get_out_port_size(); i++)
      {
         ...
         if(GetPointer<port_o>(port)->get_port_interface() == port_o::port_interface::M_AXI_AWADDR)
         {
            ...
            for(const auto& [fsymbol, func_arch] : *HLSMgr->module_arch)
            {
               const auto iface_it = func_arch->ifaces.find(bundleName);
               if(iface_it != func_arch->ifaces.end() && iface_it->second.count(FunctionArchitecture::iface_depth) &&
                  !iface_it->second.at(FunctionArchitecture::iface_depth).empty())
               {
                  queueSize = iface_it->second.at(FunctionArchitecture::iface_depth);
               }
            }
...
void LegacyTestbenchGenerationBaseStep::write_max_simulation_time_control() const
{
   ...
   writer->write(STR(STD_OPENING_CHAR));
   writer->write("begin\n");
   /// Non-blocking: currTime drives the combinational next_start_port, which is sampled both by the register
   /// starting the DUT and by the block reading the stimuli. With a blocking assignment the two posedge blocks
   /// could see different values depending on the simulator's evaluation order (a race in bambu 2023.1),
   /// starting the DUT without loading its inputs.
